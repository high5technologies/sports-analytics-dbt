    ...
    FROM
        ...
        FROM 
            (SELECT 
                ...
                , agt.w_l as away_w_l
                , hgt.w_l as home_w_l
                , agt.pre_elo_prob as away_pre_elo_prob, hgt.pre_elo_prob as home_pre_elo_prob
                , agt.pre_raptor_prob as away_pre_raptor_prob, hgt.pre_raptor_prob as home_pre_raptor_prob
                , case when l.odds_period != 'FULL' or agt.pre_carmelo_prob is null or hgt.pre_carmelo_prob is null then null 
                        when agt.pre_carmelo_prob > l.away_ml_breakeven and hgt.pre_carmelo_prob > l.home_ml_breakeven then 'B' 
                        when agt.pre_carmelo_prob < l.away_ml_breakeven and hgt.pre_carmelo_prob < l.home_ml_breakeven then 'M' 
                        when agt.pre_carmelo_prob > l.away_ml_breakeven then 'Y' else 'N' end as away_carmelo_ml_pick
                , case when l.odds_period != 'FULL' or agt.pre_carmelo_prob is null or hgt.pre_carmelo_prob is null then null 
                        when agt.pre_carmelo_prob > l.away_ml_breakeven and hgt.pre_carmelo_prob > l.home_ml_breakeven then 'B' 
                        when agt.pre_carmelo_prob < l.away_ml_breakeven and hgt.pre_carmelo_prob < l.home_ml_breakeven then 'M' 
                        when hgt.pre_carmelo_prob > l.home_ml_breakeven then 'Y' else 'N' end as home_carmelo_ml_pick
                , case when l.odds_period != 'FULL' or agt.pre_elo_prob is null or hgt.pre_elo_prob is null then null 
                        when agt.pre_elo_prob > l.away_ml_breakeven and hgt.pre_elo_prob > l.home_ml_breakeven then 'B' 
                        when agt.pre_elo_prob < l.away_ml_breakeven and hgt.pre_elo_prob < l.home_ml_breakeven then 'M' 
                        when agt.pre_elo_prob > l.away_ml_breakeven then 'Y' else 'N' end as away_elo_ml_pick
                , case when l.odds_period != 'FULL' or agt.pre_elo_prob is null or hgt.pre_elo_prob is null then null 
                        when agt.pre_elo_prob > l.away_ml_breakeven and hgt.pre_elo_prob > l.home_ml_breakeven then 'B'
                        when agt.pre_elo_prob < l.away_ml_breakeven and hgt.pre_elo_prob < l.home_ml_breakeven then 'M' 
                        when hgt.pre_elo_prob > l.home_ml_breakeven then 'Y' else 'N' end as home_elo_ml_pick
                , case when l.odds_period != 'FULL' or agt.pre_raptor_prob is null or hgt.pre_raptor_prob is null then null 
                        when agt.pre_raptor_prob > l.away_ml_breakeven and hgt.pre_raptor_prob > l.home_ml_breakeven then 'B' 
                        when agt.pre_raptor_prob < l.away_ml_breakeven and hgt.pre_raptor_prob < l.home_ml_breakeven then 'M' 
                        when agt.pre_raptor_prob > l.away_ml_breakeven then 'Y' else 'N' end as away_raptor_ml_pick
                , case when l.odds_period != 'FULL' or agt.pre_raptor_prob is null or hgt.pre_raptor_prob is null then null 
                        when agt.pre_raptor_prob > l.away_ml_breakeven and hgt.pre_raptor_prob > l.home_ml_breakeven then 'B' 
                        when agt.pre_raptor_prob < l.away_ml_breakeven and hgt.pre_raptor_prob < l.home_ml_breakeven then 'M' 
                        when hgt.pre_raptor_prob > l.home_ml_breakeven then 'Y' else 'N' end as home_raptor_ml_pick
                ...
                , l.*
                /*, case when gt.pre_carmelo_prob > ml_breakeven then 'T' else 'O' end as carmelo_ml_pick
                , case when gt.pre_elo_prob > ml_breakeven then 'T' else 'O' end as elo_ml_pick
                , case when gt.pre_raptor_prob > ml_breakeven then 'T' else 'O' end as raptor_ml_pick
                , case when (gt.pre_carmelo_prob > sbrml.ml_breakeven and gt.w_l = 'W') or (gt.pre_carmelo_prob <= sbrml.ml_breakeven and gt.w_l = 'L') then 1 else 0 end as carmelo_ml_pick_correct
                , case when (gt.pre_elo_prob > sbrml.ml_breakeven and gt.w_l = 'W') or (gt.pre_elo_prob <= sbrml.ml_breakeven and gt.w_l = 'L') then 1 else 0 end as elo_ml_pick_correct
                , case when (gt.pre_raptor_prob > sbrml.ml_breakeven and gt.w_l = 'W') or (gt.pre_raptor_prob <= sbrml.ml_breakeven and gt.w_l = 'L') then 1 else 0 end as raptor_ml_pick_correct
                */
            FROM nba.vw_transform_sbr_game_team sbr 
                inner join nba.model_game g 
                    on sbr.game_date = g.game_date 
                inner join nba.model_game_team hgt 
                    on g.game_sk = hgt.game_sk
                inner join nba.model_team ht 
                    on hgt.team_sk = ht.team_sk
                    and sbr.home_team_abbr = ht.team_abbr 
                inner join nba.model_game_team agt 
                    on g.game_sk = agt.game_sk 
                    and agt.h_a = 'A'
                inner join 
                    (SELECT aml.game_key, aml.game_date, aml.period_type as odds_period, aml.sports_book as odds_maker
                    ...
                    FROM nba.vw_raw_sbr_lines aml -- away ml
                        inner join nba.vw_raw_sbr_lines hml -- home ml
                            ...
                            and hml.h_a = 'H'
                        inner join nba.vw_raw_sbr_lines asp -- away spread
                            ...
                            and asp.h_a = 'A'
                        inner join nba.vw_raw_sbr_lines hsp -- home spread
                        ...
                        inner join nba.vw_raw_sbr_lines oaml -- open away ml
                        ...
                        inner join nba.vw_raw_sbr_lines ohml -- open home ml
                        ...
                        inner join nba.vw_raw_sbr_lines oasp -- open away spread
                        ...
                        inner join nba.vw_raw_sbr_lines ohsp -- open home spread
